#%%
# 定义Shadow模型用于训练（一个）Trigger生成器，get_poisoned获取污染的训练数据集
# 这一个trigger生成器会根据输入feat生成和target_y高度相关的trigger; 未来改为多个网络(y_t)？ 或不同的基础trigger pattern实现in the wild backdoor
...
class Backdoor:
    ...
    # 生成trigger（feat, edge）并注入图的指定节点上
    ...
    # bi-level训练Shadow GNN和TrojanNet
