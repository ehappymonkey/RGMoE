...
from models.GMI_v3 import Discriminator
import logging
import math
from attack_utils import calculate_asr_fasr_acc
from tqdm import trange
from models.model_utils import compute_diversity_loss_fast, NoisyTopKGate
from attack_utils import parse_args
...
class MoELayer(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, w_mi, w_div, topk, idx):
        # h2_experts: [N, E, d]
        h2 = [expert(x, edge_index, edge_weight) for expert in self.experts] 
        ...
        # sample nodes for the mi/div loss to enable training MoE on a single GPU
    ...
    def cal_mi_loss_fast(self, h, h2, edge_index, idx):     
        ...
        for expert_idx, mi_expert_nodes in enumerate(mi_experts):  # expert_idx in 0 ~ num_experts-1
            ...
            lengths = torch.tensor([mi.numel() for mi in mi_expert_nodes], device=mi_all.device)  
            idx = torch.repeat_interleave(torch.arange(len(lengths), device=mi_all.device),lengths) 
...
class GraphMoE(nn.Module):
    def __init__(self, in_dim, hidden_dim, num_classes, dropout, conv_type, num_experts, router, num_moe_layers=1, w_bala=1, w_div = 1, w_mi = 1):
    ...
    def forward(self, x, edge_index, edge_weight, w_mi, w_div, idx=None):
    ...
    def fit(self, poison_x, poison_edge_index, poison_edge_weights, poison_labels, induct_x, induct_edge_index,induct_edge_weights, idx_train, idx_val, idx_atk, idx_clean_test, data, train_iters=200, verbose=False, margin=0.5, args=None):
        ...
        lr_scheduler_disc = torch.optim.lr_scheduler.StepLR(
            optimizer_disc,
            step_size=100,      
            gamma=1           
        )
        ...
        for epoch in trange(0, train_iters, desc="Training", unit="epoch"):
            ...
            L_pred = F.nll_loss(log_probs[idx_train], poison_labels[idx_train])
            L_gmoe = L_pred + self.w_bala*L_balance +self.w_div*L_div
            optimizer_gmoe.zero_grad(set_to_none=True)
            # scaler.scale(L_gmoe).backward()
            # scaler.step(optimizer_gmoe)
            # scaler.update()
            L_gmoe.backward()
            optimizer_gmoe.step()
            ...
            self.eval()
            with torch.no_grad():
                output, _ = self.forward(induct_x,induct_edge_index,induct_edge_weights, w_mi=0, w_div=0)
            ...
            if epoch %5 ==0:
                ...
                asr_experts, ca_experts = self.check_every_expert(induct_x, induct_edge_index, induct_edge_weights, idx_atk, idx_clean_test, data, args)
    ...
    def check_every_expert(self, induct_x, induct_edge_index, induct_edge_weights, idx_atk, idx_clean_test, data, args, log=False):
        self.eval()
        ...
        with torch.no_grad():
            for i, expert in enumerate(self.moe_layer.experts):
                ...
                asr_i, fasr_i, ca_i = calculate_asr_fasr_acc(logits, idx_atk, idx_clean_test, data, args)
    ...
    def pred_with_random_experts(self, h, edge_index, edge_weight, K):
        ...
        num_nodes = h.size(0)
        from models.model_utils import make_random_gates
