...
import numpy as np
from rbcd_utils import parse_args
...
def set_logging(args):
    ...
    log_filename = f"logs/ptb={args.ptb_rate}/{args.dataset}_results_N={args.num_experts}.log"
...
if args.model_name == 'GMoE':
    ...
    logging.info(f"Dataset: {args.dataset}, Model: {args.model_name}, Ptb Rate: {args.ptb_rate}")
...
for seed in seeds[0:3]:
    ...
    if args.model_name == 'GMoE':
        save_dir = f"pre_trained/ptb={args.ptb_rate}/"
        ...
        if save_path.exists():
            ...
            test_model = GraphMoE(in_dim=features.shape[1],hidden_dim=32,num_classes=labels.max().item()+1,num_experts=args.num_experts, dropout=args.dropout,w_bala=args.w_bala, w_div=args.w_div,w_mi=args.w_mi).to(device)
            test_model.fit(features, edge_index, edge_weight, labels, idx_train, idx_val, idx_test)
    ...
    if args.model_name == 'GMoE':
        ...
        from label_smoothing import get_smooth_soft_label, analyze_expert_variance, PurifiedGMoE, get_smooth_soft_label_2
    ...
    print(f'Ptbed Acc of PRBCD {ptb_acc:.3f}')
